---
name: auto-research
description: Orchestrate a fully-autonomous CS/AI research pipeline from a broad topic to a top-tier-conference-style paper. Use when the user provides a research domain (e.g. "LLM hallucination", "test-time compute", "diffusion sampling") and wants the agent to run the full closed loop — literature mining, hypothesis design, code execution, and LaTeX paper drafting. Delegates each stage to a specialized sub-skill (auto-research-ideation, auto-research-method, auto-research-execution, auto-research-writing) and enforces state hand-off, hallucination guards, and human-in-the-loop checkpoints.
---

# Auto-Research Orchestrator (CS/AI)

A four-stage closed-loop research agent for computer science and artificial intelligence (LLMs, vision, RL, multimodal, systems-for-ML). The orchestrator does **not** do the research itself — it sequences four specialist sub-skills and guards the contract between them.
...
```
0. Setup       : create runs/<run_id>/ ; record domain + compute budget + deadline in run.yaml
...
```
runs/<run_id>/
├── run.yaml              # domain, budget, deadline, mode
├── stage1_ideation/
...
User: `/auto-research "test-time compute scaling for small LMs", budget=40 GPU-hours on 1×H100, deadline=14 days, autonomous`

The orchestrator should:

1. `mkdir -p runs/2026-05-06-ttc-small-lm/`
2. Write `run.yaml` with the parsed arguments.
3. Invoke `auto-research-ideation` with the domain + budget context.
4. Score the 3 candidates by feasibility-under-budget × novelty (since `--autonomous`); pick the top one.
5. Invoke `auto-research-method` on the chosen idea.
6. **Stop.** Even in autonomous mode, do not spend GPU until the user has at least seen `experiment_plan.yaml`. Display it and ask: "Approve plan? [y/N/edit]"
7. On approval, invoke `auto-research-execution`.
8. Stream a one-line status per epoch to the user; do not silently consume budget.
9. Invoke `auto-research-writing`.
10. Final deliverable: `runs/.../stage4_writing/paper.pdf` + a short summary message linking to it.
